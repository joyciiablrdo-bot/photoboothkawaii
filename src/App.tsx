...
.home-hamburger {
  ...
  padding: .4rem; flex-shrink: 0; margin-left: auto;
}
.home-hamburger span {
  display: block; width: 22px; height: 3px;
  background: var(--pink-dark); border-radius: 3px;
  transition: transform .3s, opacity .3s;
}
.home-hamburger.open span:nth-child(1) { transform: translateY(8px) rotate(45deg); }
.home-hamburger.open span:nth-child(2) { opacity: 0; }
.home-hamburger.open span:nth-child(3) { transform: translateY(-8px) rotate(-45deg); }
@media (max-width: 700px) {
  ...
  .nav-links {
    ...
    border-radius: 0 0 24px 24px;
    padding: 12px 0 18px;
    box-shadow: 0 8px 24px rgba(255,133,161,0.25);
  ...
  .nav-links a { font-size: 1rem; padding: 8px 20px; display: block; text-align: center; }
  .home-hamburger { display: flex; }
  .nav-bow { display: none; }
}
.nav-logo {
  font-family: 'Nunito', sans-serif;
  ...
  return (
    <nav>
      ...
      <ul className={`nav-links${menuOpen ? " open" : ""}`}>
        ...
        <li><Link to="/photobooth" className={location.pathname === "/photobooth" ? "active" : ""} onClick={() => setMenuOpen(false)}>Photobooth</Link></li>
        <li><Link to="/contact" className={location.pathname === "/contact" ? "active" : ""} onClick={() => setMenuOpen(false)}>Contact</Link></li>
        <li><Link to="/privacy" className={location.pathname === "/privacy" ? "active" : ""} onClick={() => setMenuOpen(false)}>Privacy Policy</Link></li>
      </ul>
      <span className="nav-bow">🎀</span>
      <button className={`home-hamburger${menuOpen ? " open" : ""}`} aria-label="Toggle menu" onClick={() => setMenuOpen(p => !p)}>
        <span /><span /><span />
      </button>
    </nav>
  );
